...
core::Result<models::ProgramConfig> ConfigManager::parse(std::span<const char*> args) {
    ...
    // Default values
    std::string input_path = "data/lena.png";
    ...
    auto cli = lyra::cli()
        | lyra::help(help)
        | lyra::opt(input_path, "input")
            ["-i"]["--input"]
            ("Path to input image (default: data/lena.png)")
        | lyra::opt(output_path, "output")
            ["-o"]["--output"]
            ("Path to output image (default: data/output.png)")
        | lyra::opt(type_str, "type")
            ["-t"]["--type"]
    ...
    if (type_str == "simple") {
        program_type = models::ProgramType::Simple;
    } else if (type_str == "grayscale") {
        program_type = models::ProgramType::Grayscale;
    } else {
        return core::Result<models::ProgramConfig>::error(
            "Invalid program type: '" + type_str + 
            "'. Must be 'simple' or 'grayscale'."
        );
    }
    
    // Return configuration successfully
    models::ProgramConfig config{
        .input_image_path = input_path,
        .output_image_path = output_path,
        .program_type = program_type
    };
    
    return core::Result<models::ProgramConfig>::ok(
        std::move(config),
