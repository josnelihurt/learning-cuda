...
class CudaGrayscaleProcessorTest : public ::testing::Test {
protected:
  void SetUp() override {
    image_loader_ = std::make_unique<ImageLoader>();
    ASSERT_TRUE(image_loader_->is_valid());
    processor_ = std::make_unique<GrayscaleProcessor>();
  ...
  std::unique_ptr<GrayscaleProcessor> processor_;
  std::unique_ptr<TestImageSink> sink_;
};

TEST_F(CudaGrayscaleProcessorTest, ConstructsWithDefaultAlgorithm) {
  // Arrange & Act
...
TEST_F(CudaGrayscaleProcessorTest, ConstructsWithCustomAlgorithm) {
  // Arrange & Act
  GrayscaleProcessor processor(GrayscaleAlgorithm::BT709);

  // Assert
  EXPECT_EQ(processor.get_algorithm(), GrayscaleAlgorithm::BT709);
}

TEST_F(CudaGrayscaleProcessorTest, SetAlgorithmUpdatesAlgorithm) {
  // Arrange
  GrayscaleProcessor processor;
...
TEST_F(CudaGrayscaleProcessorTest, ProcessConvertsImageToGrayscale) {
  // Arrange
  ASSERT_TRUE(image_loader_->is_valid());
...
TEST_F(CudaGrayscaleProcessorTest, ProcessPreservesImageDimensions) {
  // Arrange
  ASSERT_TRUE(image_loader_->is_valid());
...
TEST_F(CudaGrayscaleProcessorTest, DifferentAlgorithmsProduceDifferentResults) {
  // Arrange
  GrayscaleProcessor processor1(GrayscaleAlgorithm::BT601);
  GrayscaleProcessor processor2(GrayscaleAlgorithm::BT709);
...
TEST_F(CudaGrayscaleProcessorTest, ProcessWithAverageAlgorithm) {
  // Arrange
  GrayscaleProcessor processor(GrayscaleAlgorithm::Average);
  TestImageSink sink;
...
TEST_F(CudaGrayscaleProcessorTest, ProcessWithLightnessAlgorithm) {
  // Arrange
  GrayscaleProcessor processor(GrayscaleAlgorithm::Lightness);
  TestImageSink sink;
...
TEST_F(CudaGrayscaleProcessorTest, ProcessWithLuminosityAlgorithm) {
  // Arrange
  GrayscaleProcessor processor(GrayscaleAlgorithm::Luminosity);
  TestImageSink sink;
...
TEST_F(CudaGrayscaleProcessorTest, OutputDataIsValidGrayscale) {
  // Arrange
  ASSERT_TRUE(image_loader_->is_valid());
