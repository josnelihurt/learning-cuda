...
if [ "$SKIP_GOLANG" = false ]; then
  ...
  go test -race $(go list ./... | grep -v "src/go_api/pkg/infrastructure/processor/loader" | grep -v "test/integration/tests" | grep -v "src/tools/generate-video-metadata" | grep -v "proto/gen") || {
