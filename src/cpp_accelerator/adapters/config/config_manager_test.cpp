#include "src/cpp_accelerator/adapters/config/config_manager.h"
...
TEST_F(ConfigManagerTest, ParseWithDefaultValues) {
  ...
  EXPECT_EQ(uut_result.value->input_image_path, "data/static_images/lena.png");
  EXPECT_EQ(uut_result.value->output_image_path, "data/output.png");
  EXPECT_EQ(uut_result.value->program_type, models::ProgramType::CudaImageFilters);
}
...
TEST_F(ConfigManagerTest, ParseWithSimpleProgramType) {
  ...
  ASSERT_TRUE(uut_result.success);
  ASSERT_TRUE(uut_result.value.has_value());
  EXPECT_EQ(uut_result.value->program_type, models::ProgramType::Passthrough);
}
...
TEST_F(ConfigManagerTest, ParseWithAllCustomParameters) {
  // Arrange
  std::vector<std::string> args = {"program",       "-i", "my_input.png", "-o",
                                   "my_output.png", "-t", "simple"};
  auto args_span = make_args(args);
...
TEST_F(ConfigManagerTest, ParseWithLongOptionNames) {
  // Arrange
  std::vector<std::string> args = {"program",         "--input", "long_input.png", "--output",
                                   "long_output.png", "--type",  "simple"};
  auto args_span = make_args(args);
