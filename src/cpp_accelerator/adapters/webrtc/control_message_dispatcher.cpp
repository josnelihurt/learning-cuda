...
ControlMessageDispatcher::ControlMessageDispatcher(Config config)
  ...
  handlers_[ControlRequest::kListFilters] = [this](const auto& req, const auto& /*sid*/,
                                                   auto& /*state*/,
                                                   auto* resp) { HandleListFilters(req, resp); };
  handlers_[ControlRequest::kGetVersion] = [this](const auto& req, const auto& /*sid*/,
                                                  auto& /*state*/,
                                                  auto* resp) { HandleGetVersion(req, resp); };
  ...
  handlers_[ControlRequest::kStartCameraStream] = [this](const auto& req, const auto& sid,
                                                         auto& state, auto* resp) {
    HandleStartCameraStream(req, sid, state, resp);
  };
  handlers_[ControlRequest::kStopCameraStream] = [this](const auto& /*req*/, const auto& sid,
                                                        auto& state, auto* resp) {
    HandleStopCameraStream(sid, state, resp);
  };
  handlers_[ControlRequest::kCaptureFrame] = [this](const auto& /*req*/, const auto& sid,
                                                    auto& state, auto* resp) {
    HandleCaptureFrame(sid, state, resp);
  };
  handlers_[ControlRequest::kListCapturedImages] = [this](const auto& req, const auto& sid,
                                                          auto& /*state*/, auto* resp) {
    HandleListCapturedImages(req, sid, resp);
  };
  handlers_[ControlRequest::kGetCapturedImage] = [this](const auto& req, const auto& sid,
                                                        auto& /*state*/, auto* resp) {
    HandleGetCapturedImage(req, sid, resp);
  };
  handlers_[ControlRequest::kDeleteCapturedImage] = [this](const auto& req, const auto& sid,
                                                           auto& /*state*/, auto* resp) {
...
void ControlMessageDispatcher::HandleGetAcceleratorCapabilities(const ControlRequest& /*request*/,
                                                                ControlResponse* response) {
  ...
  caps_resp->set_version(accelerator_version_);
...
void ControlMessageDispatcher::HandleStartCameraStream(const ControlRequest& request,
  ...
  auto frame_cb = [handler = video_frame_handler_, sid = session_id](const rtc::binary& data,
                                                                     const rtc::FrameInfo& info) {
    handler(sid, data, info);
  };

  std::string cam_err;
  auto subscription = camera_hub_->Subscribe(req.sensor_id(), req.width(), req.height(), req.fps(),
                                             std::move(frame_cb), &cam_err);
...
void ControlMessageDispatcher::HandleCaptureFrame(const std::string& session_id,
                                                  SessionState& state, ControlResponse* response) {
  ...
  std::string filename = std::string("capture_") + ts + "_" + std::to_string(ms.count()) + ".bmp";
  std::error_code ec;
...
void ControlMessageDispatcher::HandleGetCapturedImage(const ControlRequest& request,
  ...
  if (req.format() == cuda_learning::CAPTURED_IMAGE_FORMAT_BMP) {
    ...
    spdlog::info("[WebRTC:{}] GetCapturedImage(PNG) {} {}x{} {} bytes", session_id, req.id(), w, h,
                 img_data.size());
  }
}
...
void ControlMessageDispatcher::HandleDeleteCapturedImage(const ControlRequest& request,
  ...
  if (!removed || ec) {
    ...
    spdlog::warn("[WebRTC:{}] DeleteCapturedImage failed: {} \xe2\x80\x94 {}", session_id, req.id(),
