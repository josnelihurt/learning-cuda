# CUDA Infrastructure — A Tutor's Walkthrough

This folder contains every GPU kernel and GPU-side helper used by the accelerator. The code here is the "infrastructure" layer of clean architecture: the actual hardware-specific implementation behind the abstract `IFilter` / `IImageProcessor` ports defined in `domain/`.
...
graph TB
    ...
    subgraph Device[Device / GPU]
        POOL[CudaMemoryPool<br/>thread-local cache]
        K1[grayscale_kernel.cu]
        K2[blur_kernel.cu<br/>1D / 2D / separable / tiled]
        K3[letterbox_kernel.cu]
        TRT[TensorRT YOLO engine]
    end
...
| File | Purpose |
|---|---|
| `cuda_memory_pool.{h,cpp}` | Thread-local GPU allocation cache. Kills `cudaMalloc/Free` churn. |
| `grayscale_kernel.cu` + `grayscale_filter.{h,cpp}` | RGB → 1-channel luma, 5 algorithms. |
| `blur_kernel.cu` + `blur_processor*` | Gaussian blur, four flavors (full 2D, two 1D passes, separable+tiled). |
| `letterbox_kernel.cu` | Aspect-preserving resize + pad + uint8→float NCHW for TRT input. |
| `yolo_detector.{h,cpp}` + `model_*`, `yolo_factory*` | TensorRT-based YOLO inference + NMS post-process. |
...
## 3. `cuda_memory_pool` — Why we don't `cudaMalloc` per frame
...
## 4. `grayscale_kernel.cu` — The Hello-World
...
## 5. `blur_kernel.cu` — Four Flavors of Gaussian Blur
...
## 6. `letterbox_kernel.cu` — Resize + Pad + Format Convert in One Pass
...
## 7. `yolo_detector.cpp` — TensorRT End-to-End
...
- **`__global__`, `__device__`, `__constant__` qualifiers** — `blur_kernel.cu`.
...
- **Memory pool / reuse** — `cuda_memory_pool.cpp`.
- **NCHW layout for DL inference** — letterbox kernel scatter write.
- **TensorRT integration** — `yolo_detector.cpp`: `IRuntime`, `ICudaEngine`, `IExecutionContext`, `enqueueV3`, `setTensorAddress`, `setMemoryPoolLimit`, `buildSerializedNetwork`, ONNX parser, optimization profile for dynamic shapes.
